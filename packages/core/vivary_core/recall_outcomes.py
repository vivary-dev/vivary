...
these values. A normalized candidate with no recalled match is ``accepted`` with an
empty ``reason_codes`` list; every other ordinary evaluation condition has its
specified label. The frozen Node oracle used different outcome names; this module
...
# Labeled SPEC §6.2 conditions. Results carry these values in ``reason_codes`` to
# retain the repository-wide machine-readable decision shape; the accepted no-match
# result deliberately carries an empty list.
...
REASON_CORRECTION_TARGET_MISMATCH = "correction_target_mismatch"

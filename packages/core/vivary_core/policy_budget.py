...
- JS `typeof limits.max_turns === "number"` -> ``isinstance(..., (int,
  float)) and not isinstance(..., bool)`` (Python bool is an int subclass;
  JS has no such overlap, so a stray boolean must not masquerade as a
  numeric limit).
"""

...
against the linked manifests, contracts, and registry pages on **2026-08-09**.
...
| **Experimental** | Implemented in development source, opt-in where callable, and not registry-available at the source versions. | `vivary-core`; governed Tropo, Strato, Ozone, and Exo paths; the provider-neutral recall firewall; and the optional read-only `vivary-mcp` adapter. | [Core contract tests](https://github.com/vivary-dev/vivary/tree/dev/packages/core/tests), [role envelopes](/commands/#governed-machine-readable-envelopes-development-source), [memory firewall spec](https://github.com/vivary-dev/vivary/blob/dev/docs/bellamente-memory/SPEC-bellamente-memory.md#6-candidaterecallprovider-contract), and [MCP contract tests](https://github.com/vivary-dev/vivary/tree/dev/packages/mcp/tests). |
| **Held** | Complete or partial source work that must not be described as published, graduated, or default-enabled. | Publication of the named **Vivary Governed Context** train, including every unpublished source version in the root release status. | Open release policy [#149](https://github.com/vivary-dev/vivary/issues/149), documentation program [#210](https://github.com/vivary-dev/vivary/issues/210), and the per-item human gates in the [release workflow](/release-workflow/#gates). |
| **Deprecated** | Formally discouraged with a documented replacement and removal policy. | **None.** The `legacy-v0.1` flat workspace shape and legacy Exo graph commands remain compatibility surfaces, not deprecations. | [Doctor compatibility contract](/commands/#doctor-compatibility-and-declared-configuration) and [legacy Exo commands](/commands/#legacy-graph-coordination). |

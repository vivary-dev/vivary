...
## [Unreleased: full-train artifact proof] — 2026-08-13

Published registry versions and the staged package versions remain unchanged.

### Added

- Main CI now builds the complete coordinated train: nine Python wheels, nine source
  distributions, and the `@vivary/create` tarball. The gate rejects missing or extra
  artifacts, package identity drift, license drift, tests inside wheels, private
  release evidence or root escapes inside source distributions, and unexpected npm
  payloads.
- The exact candidate wheel graph is installed with the registry disabled. The same
  graph is then rebuilt and installed from source distributions with the reviewed
  `setuptools==84.0.0` backend, and the packed npm tarball is installed and executed
  against the exact local `create-vivary` wheel.
- CI contract mutations now fail when any distribution build, archive inspection,
  install smoke, dependency check, packed-launcher execution, or required ordering is
  removed.

### Changed

- The root verification router no longer copies volatile test counts; exit status is
  the durable contract and current evidence remains in the changelog and CI.

### Verification

- `python scripts/tests/test_release_artifacts.py` — 14 positive and mutation cases
  passed.
...
- `python scripts/check_release_artifacts.py --repository . --artifacts
  <candidate-artifacts>` — all 19 candidate archives passed full-train inspection.
- An isolated Python 3.11 environment installed the `vivary==0.1.10` graph from the
  candidate source distributions with the registry disabled, source archives forced,
  and build isolation disabled, then passed `uv pip check` and exact version assertions.
- `python scripts/check_package_docs_parity.py`, its 10-case regression suite, the
  repository automation guard and its 11-case suite, the npm trusted-publish guard,
  the line-ending guard, and `git diff --check` passed.
- From `site/`, `npm ci` audited 360 packages with zero vulnerabilities; `npm run
  build` generated 33 pages, the 14 site behavior tests passed, and the link checker
  validated 2,729 local references plus 1,531 anchors with zero failures.

Publishing remains a manual human gate. Benchmark, website dogfood, registry setup,
tagging, and publication remain outside this slice.

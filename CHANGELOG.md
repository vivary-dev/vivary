...
- `vivary <verb> --help` now prints `usage: vivary <verb> ...`, and a usage error from
  a routed verb reads `vivary <verb>: error: ...`. The router passes the program name
  only to a component whose `main` accepts it, so an older component keeps the
...
- `python packages/vivary/tests/test_command_surface_characterization.py` — 3/3 passed
  over the frozen surface table, which is unchanged by this slice.
- `python packages/vivary/tests/test_vivary_router.py` — 24/24 passed.
- `python scripts/tests/test_installed_route_parity.py` — 24/24 passed.
...
- `python scripts/tests/test_release_artifacts.py` — 7/7 passed, and
  `python scripts/check_release_artifacts.py --repository . --artifacts <dir>` over
  freshly built `create-vivary`, `vivary-mcp`, and `vivary` distributions plus
  `npm pack` of the launcher — 7 release artifacts passed license verification.
- Nine wheels built with `python -m pip wheel --no-deps`, installed into a fresh
  ...
  help parity, 8 fixture parity, and 1 unrouted operation passed — and
  `... --characterize <venv>/bin` — 37 installed command-surface cases passed.
- `node packages/create-vivary/tests/test_npm_launcher.js` — 11/11 passed. The npm
  ...
  audit there.
- By hand against that installed environment: `vivary check --help`,
  `vivary create --help`, `vivary decide --help`, and `vivary control --help` each
  print `usage: vivary <verb> ...`. `vivary check --typo` prints
  `vivary check: error: unrecognized arguments: --typo` and exits `2`, and

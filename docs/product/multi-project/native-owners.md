...
| Code and Desktop runs | Code run store, executor, transcript, run controls, `CodeAgentsHost`, and Desktop remote dispatch | Select either Code or Harness ownership. Add a host bridge only after its execution environment passes compatibility checks | `mapped from installed exports` and `documented only`: `code-agents-ui.mdx`. Shared UI package availability remains unverified |
